...
dados a partir de TODAS as fichas do imóvel (assinadas ou não), com o telefone
mascarado.
"""
...
def test_montar_dados_visitas_inclui_fichas_nao_assinadas():
    """O relatório sai de TODAS as fichas do imóvel, assinadas ou não, com
    endereço e datas formatados."""
    ...
    # Não filtra por status: fichas não assinadas entram no relatório também.
    db.eq.assert_called_once_with("imovel_id", IMOVEL["id"])
    ...
    # Ficha sem `assinada_em` (não assinada) cai no created_at, para nunca sair sem data.
