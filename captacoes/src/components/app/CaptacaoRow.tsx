...
import { ChevronDown, ChevronUp, GripVertical, ImageOff, MapPin, MessageSquare } from "lucide-react";
...
/** Chips de especificação, na ordem de leitura do handoff. Zero some. */
...
export function CaptacaoRow({
  ...
  return (
    <div className="flex gap-3 rounded-2xl border bg-card p-3 shadow-[0_1px_2px_rgba(46,48,42,0.04),0_10px_24px_-18px_rgba(46,48,42,0.22)]">
      {emSequencia && (
        <div className="flex flex-none flex-col items-center justify-center gap-0.5 text-muted-foreground">
          <button
            type="button"
            onClick={onSubir}
            disabled={!onSubir}
      ...
      <Link href={`/captacao/${captacao.id}`} className="flex min-w-0 flex-1 gap-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1.5">
          <div className="flex items-start justify-between gap-2">
            ...
            {opinioes?.naoLidas ? (
              <span className="flex flex-none items-center gap-1 rounded-md bg-primary/25 px-1.5 py-0.5 text-[10.5px] font-bold text-[#857727]">
