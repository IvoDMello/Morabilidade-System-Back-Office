...
describe("whatsappLink", () => {
  it("limpa o telefone e gera URL wa.me correta", () => {
    const url = whatsappLink("(21) 99999-9999", { codigo: "MB-00001", bairro: "Humaitá" });
    expect(url).toContain("wa.me/21999999999");
  });

  it("inclui tipo do imóvel no texto", () => {
    const url = decodeURIComponent(
      whatsappLink("21999999999", { codigo: "MB-00001", bairro: "Humaitá", tipo_imovel: "apartamento" })
    );
  ...
  it("inclui dormitórios quando presentes", () => {
    const url = decodeURIComponent(
      whatsappLink("21999999999", { codigo: "MB-00001", bairro: "Ipanema", dormitorios: 3 })
    );
  ...
  it("funciona sem campos opcionais", () => {
    const url = whatsappLink("21999999999", { codigo: "MB-00001", bairro: "Centro" });
    expect(url).toContain("wa.me/21999999999");
    expect(url).toContain("MB-00001");
    expect(url).toContain("Centro");
  });

  it("inclui o código do imóvel no texto", () => {
    const url = decodeURIComponent(
      whatsappLink("21999999999", { codigo: "MB-00042", bairro: "Flamengo" })
    );
    expect(url).toContain("MB-00042");
  });
});
